...
- アプリ編集画面を整理。監視タブを「終了したときの再起動」「応答なし検出」「再起動ループ防止」の枠に分け、ループ防止の現在の設定を文章で表示。
- 「終了時の動作」の選択肢を、実際の動作が分かる表現に変更（アプリのウィンドウを閉じた場合も再起動の対象になることを明記）。
- 回数を数える期間・上限に達したときの停止時間など、あいまいだった項目名を変更。
- 使われない設定をグレーアウト（再起動しない設定のときの待機時間・ループ防止、無効時の応答なし検出の各項目など）。
- 入力の誤りを項目の横と画面下部にその場で表示し、「設定を確認」ボタンを廃止。重複登録も画面を閉じずに知らせる。
- 待機0秒かつ回数制限なしの設定に警告を表示し、保存時に確認。
- 動作中のアプリは実行ファイルと実行権限を読み取り専用にし、編集画面から停止して変更できるようにした（保存後に拒否されて入力が失われる問題を解消）。
- 1択だった「子プロセスの扱い」の選択欄と、開発者向けの起動方式の説明を削除。空白の多かった画面の高さを縮小。

## 0.1.0-alpha.25

- 実行中アプリ一覧の画面で、説明文の幅まで画面全体が広がり、右下のボタンがウィンドウ外にはみ出していた問題を修正。

## 0.1.0-alpha.24

- AgentとElevatedが30秒ごとに互いの応答を確認し、2回続けて応答がなければ登録済みタスクから相手を起動し直す。一度も応答を確認できていない相手は起動せず、起動し直すのは1時間に3回まで。
- 捕捉されなかった例外（UIスレッド・バックグラウンドタスク・プロセス終了時）をfallbackログへ記録。
- トレイの表示に管理者権限アプリの状態を含め、管理者Helperに接続できない場合も異常として表示。
- 自動再起動・Backoff・応答なし・起動失敗・管理者Helperの切断をトレイ通知で知らせる。設定画面で無効化できる。
- ダッシュボードの更新間隔を設定画面から変更可能にし（`global.uiRefreshSeconds`、1〜60秒）、最小化中は更新を止める。設定ファイルは変更があったときだけ読み直す。
- 終了要求の確認を200ms間隔のタイマーから待機ハンドル・タスク完了通知へ変更し、常時の定期起床をなくした。
- 外部プロセスの定期探索で、プロセス一覧の取得を1周期1回に共有。探索対象がなければ一覧を取得しない。
- AgentとElevatedが同時にログ整理を始めた場合の`LogMaintenanceFailed: TimeoutException`記録をなくした（後から来た側は整理をスキップする）。
- 未使用だった`global.checkForUpdates`を削除（既存設定に残っていても読み込みは失敗しない）。
- GitHub Actionsのアクションを、Node.js 24で動く版へ更新。

## 0.1.0-alpha.23

- 自動起動タスクの優先度を「通常」で登録。従来の既定値（BelowNormal）では、AppWatcherが再起動したアプリも低優先度で動作していた。旧登録はセルフチェックで警告し、修復を案内する。旧タスクから起動したホストでも、起動したアプリの優先度は通常へ戻す。
- 応答なし時の動作（`hangAction`）を追加。`LogOnly`（記録のみ）と `Restart`（強制終了して再起動）を選べる。新規登録の既定は `LogOnly`。
- 設定スキーマを3へ更新。既存設定で応答なし検出が有効なアプリは、従来と同じ `Restart` へ移行する。
- 自動再起動の直前に既存インスタンスを再確認し、待機中にユーザーやアプリ自身が起動したインスタンスがあれば、新たに起動せずそのインスタンスを監視する（`AttachExisting` 有効時）。
- 既存プロセスの照合を、パスに加えて権限レベル・ユーザー・セッションまで一致させる方式へ変更。照合には `MainModule` ではなく `QueryFullProcessImageName` を使う。
